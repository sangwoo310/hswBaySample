...
void nftbay::create(name issuer, std::string sym)
{
    require_auth(_self);
    ...
    currency_table.emplace(_code, [&](auto &currency) {
        currency.supply = supply;
        currency.issuer = issuer;
    });
}

void nftbay::issue(name to, std::string sym, uint64_t index)
{
    checkpause(to);
    ...
    if (symbols == symbol("UTS", 0))
    {
        ...
        stokens.emplace(name{CONTRACT_ACCOUNT}, [&](auto &token) {
    ...
    if (symbols == symbol("UTM", 0))
    {
        ...
        monster_table monster(GAME_CONTRACT, to.value);
        auto monster_iter = monster.get(index, "Not exist Monster");

        auto utm_list = mtokens.get_index<"bymaster"_n>();
        bool not_exist = true;
        for (auto it = utm_list.begin(); it != utm_list.end(); ++it)
        {
            if (it->t_idx == monster_iter.index && it->master == to)
            {
                not_exist = false;
                break;
            }
        ...
        mtokens.emplace(name{CONTRACT_ACCOUNT}, [&](auto &token) {
    ...
    if (symbols == symbol("UTI", 0))
    {
        ...
        item_table item(GAME_CONTRACT, to.value);
        auto item_iter = item.get(index, "Not exist Item");

        auto uti_list = itokens.get_index<"bymaster"_n>();
        bool not_exist = true;
        for (auto it = uti_list.begin(); it != uti_list.end(); ++it)
        {
            if (it->t_idx == item_iter.index && it->master == to)
            {
                not_exist = false;
                break;
            }
        ...
        itokens.emplace(name{CONTRACT_ACCOUNT}, [&](auto &token) {
...
void nftbay::transferid(name from, name to, id_type id, std::string sym)
{
    ...
    if (token.symbol == symbol("UTS", 0))
    {
        ...
        stokens.modify(st, _code, [&](auto &token) {
            token.owner = to;
        });
    ...
    if (token.symbol == symbol("UTM", 0))
    {
        ...
        mtokens.modify(st, _code, [&](auto &token) {
            token.owner = to;
        });
    ...
    if (token.symbol == symbol("UTI", 0))
    {
        ...
        itokens.modify(st, _code, [&](auto &token) {
            token.owner = to;
        });
...
void nftbay::backtogame(name from, std::string sym, id_type id)
{
    checkpause(from);
    ...
    if (token.symbol == symbol("UTS", 0))
    {
        ...
        stokens.modify(st, _code, [&](auto &data) {
            data.state = "delete";
        });

        servant_table user_servant_table(GAME_CONTRACT, target_token->master.value);
        auto user_servant_iter = user_servant_table.find(target_token->t_idx);
        eosio_assert(user_servant_iter != user_servant_table.end(), "This servant is not exist.");

        icon_id = user_servant_iter->servant.id;
    }
    if (token.symbol == symbol("UTM", 0))
    {
        type = "monster";
        ...
        mtokens.modify(st, _code, [&](auto &data) {
            data.state = "delete";
        });

        monster_table user_monster_table(GAME_CONTRACT, target_token->master.value);
        auto user_monster_iter = user_monster_table.find(target_token->t_idx);
        eosio_assert(user_monster_iter != user_monster_table.end(), "This servant is not exist.");

        icon_id = user_monster_iter->monster.id;
    }
    if (token.symbol == symbol("UTI", 0))
    {
        type = "item";
        ...
        itokens.modify(st, _code, [&](auto &data) {
            data.state = "delete";
        });

        item_table user_equipment_table(GAME_CONTRACT, target_token->master.value);
        auto user_equipment_iter = user_equipment_table.find(target_token->t_idx);
        eosio_assert(user_equipment_iter != user_equipment_table.end(), "This servant is not exist.");

        icon_id = user_equipment_iter->equipment.id;
    }

    action(permission_level{get_self(), "active"_n},
           GAME_CONTRACT, "nftmail"_n,
           std::make_tuple(from, type, id, icon_id))
        .send();
}

void nftbay::deletedata(std::string sym, id_type id)
{
    checkpause(GAME_CONTRACT);
...
void nftbay::sub_balance(name owner, eosio::asset value)
{
    ...
    {
        from_acnts.modify(from, _code, [&](auto &a) {
            a.balance -= value;
        });
    }
}

void nftbay::add_balance(name owner, eosio::asset value, name ram_payer)
{
    account_index to_accounts(_self, owner.value);
    auto to = to_accounts.find(value.symbol.code().raw());
    if (to == to_accounts.end())
    {
        to_accounts.emplace(name{CONTRACT_ACCOUNT}, [&](auto &a) {
            a.balance = value;
        });
    }
    else
    {
        to_accounts.modify(to, name{CONTRACT_ACCOUNT}, [&](auto &a) {
            a.balance += value;
        });
    }
}

void nftbay::sub_supply(eosio::asset quantity)
{
    auto symbol_name = quantity.symbol.code().raw();
    currency_index currency_table(_self, symbol_name);
    auto current_currency = currency_table.find(symbol_name);

    currency_table.modify(current_currency, name{CONTRACT_ACCOUNT}, [&](auto &currency) {
        currency.supply -= quantity;
    });
}

void nftbay::add_supply(eosio::asset quantity)
{
    auto symbol_name = quantity.symbol.code().raw();
    currency_index currency_table(_self, symbol_name);
    auto current_currency = currency_table.find(symbol_name);
...
void nftbay::regauction(name seller, uint64_t token_id, asset min_price, asset max_price, std::string sym)
{
    checkpause(seller);
    require_auth(seller);
    ...
    regauction_core(seller, master, token_id, min_price, max_price, nftType, game_table_id);
}

void nftbay::regauction_core(name seller, name master, uint64_t token_id, asset min_price, asset max_price, uint64_t nft_type, uint64_t game_table_id)
{
...
void nftbay::updatenftstate(name seller, uint64_t nft_type, uint64_t token_id, std::string state)
{
    //UTS
    if (nft_type == 1)
    {
...
// 만료시간까지 판매가 안되었으면 취소
void nftbay::cancelauc(name seller, uint64_t trade_id)
{
    require_auth(seller);
    checkpause(seller);
    ...
    if (iterator != trade_table.end())
    {
        ...
        eosio_assert(iterator->seller == seller, "owner account does not match.");
        eosio_assert(cur_time > iterator->end_bid_time, "you can not cancel.");

        trade_table.modify(iterator, _code, [&](auto &row) {
...
void nftbay::successbid(name seller, name bid_winner, uint64_t nft_type, uint64_t token_id)
{
    checkpause(seller);
...
void nftbay::notify(name user, std::string msg)
{
    require_auth(get_self());
    require_recipient(user);
}

void nftbay::claim(std::string actor, name account, uint64_t trade_id)
{
    ...
    eosio_assert(iterator->state == STATE_SELLING, "Wrong State.");
    ...
    if (iterator != trade_table.end())
    {
        ...
        // 거래 시간이 안끝났으면 오류
        if (iterator->end_bid_time > cur_time)
        {
        ...
        {
            if (iterator->seller != account || iterator->seller == iterator->owner)
            {
                eosio_assert(false, "Bad request");
            }
        }
        ...
        action(permission_level{get_self(), "active"_n},
               ...
               std::make_tuple(_self, iterator->seller, soldPrice, sold_memo))
            .send();
...
void nftbay::sendsummary(name user, std::string message)
{
    action(permission_level{get_self(), "active"_n},
           get_self(),
           "notify"_n,
...
// 메모 형식
// type:to_account:trade_id (bid, buyitnow)
// type:to_account:symbol:token_table_index (issue)
// * type : bid, buyitnow, refund, issue
// ex) bid:kylinaccount:23
// ex) buyitnow:kylinaccount:11
// ex) issue:kylinaccount:UTS:3
void nftbay::bid()
{
    ...
    if (transfer_data.from != _self) //액션을 실행시키는 주체가 내가 아니면
    {
        if (transfer_data.to == _self) //액션 실행시 받는 대상이 컨트랙트의 경우
        {
            checkpause(transfer_data.from);
            if (tokens.size() == 3)
            {
                ...
                if (trans_type == "bid" || trans_type == "buyitnow")
                {
                    ...
                    if (iterator != trade_table.end())
                    {
                        ...
                        name seller_name = iterator->seller;
                        ...
                        eosio_assert(seller_name == seller, "seller account does not match.");
                        eosio_assert(seller_name != buyer, "seller cannot be a buyer.");
                        ...
                        if (trans_type == "bid")
                        {
                            ...
                            // refund eos
                            if (target_name != seller_name)
                            {
                                std::string refund_memo;
                                //refund_memo.append("nftbay_refund:", trade_id);
                                refund_memo += "nftbay_refund:";
                                refund_memo += std::to_string(trade_id);
                        ...
                        else if (trans_type == "buyitnow")
                        {
                            if (max_price.amount == bid_price.amount)
                            {
                                ...
                                successbid(seller_name, buyer, nft_type, iterator->token_id);
                                ...
                                std::string sold_memo;
                                //sold_memo.append("nftbay_sold:", trade_id);
                                sold_memo += "nftbay_sold:";
                                sold_memo += std::to_string(trade_id);
            ...
            else if (tokens.size() == 4)
            {
                ...
                if (trans_type == "issue")
                {
                    name to = name{tokens[1]};
                    std::string sym = tokens[2];
                    const char *tidx_char = tokens[3].c_str();
                    uint64_t t_idx = std::atoi(tidx_char);
                    auto t_owner = transfer_data.from;
                    auto issue_price = transfer_data.quantity;

                    eosio_assert(to == t_owner, "The sender and the issuer are different.");
                    eosio_assert(issue_price.amount == 500, "Only 0.05 EOS is needed.");

                    issue(to, sym, t_idx);
                }
                else
                {
                    eosio_assert(false, "Memo format is wrong.");
                }
            }
            else
            {
                eosio_assert(false, "Token Size Error");
            }
        }
        else
        {
            eosio_assert(false, "Wrong Action");
        }
    }
    else //액션을 실행시키는 주체가 나면
    {
        require_auth(_self);
        if (transfer_data.to == "eosio.ram"_n)
        {
        }
        else if (transfer_data.to == "eosio.ramfee"_n)
        {
        }
        else if (transfer_data.to == "eosio.stake"_n)
        {
        }
        else if (transfer_data.to == "b.i.t.e"_n)
        {
        }
        else if (tokens[0] == "nftbay_sold")
        {
        }
        else if (tokens[0] == "nftbay_refund")
        {
        }
        else
        {
            eosio_assert(false, "Impossible Send");
        }
    }
}
...
void nftbay::tokenize(const std::string &str,
                      std::vector<std::string> &tokens,
...
void nftbay::addwhitelist(name user)
{
    require_auth(_code);
    ...
    whitelist.emplace(_code, [&](auto &newuser) {
        newuser.user = user;
    });
}

void nftbay::delwhitelist()
{
    require_auth(_code);
    white_index whitelist(_self, _self.value);
...
void nftbay::setpause(uint64_t _state)
{
    require_auth(_self);
    master_index master_table(_self, _self.value);
    auto master_iter = master_table.begin();
    if (master_iter == master_table.end())
    {
        master_table.emplace(_code, [&](auto &new_data) {
            new_data.master = _self;
            new_data.state = system_state::pause;
        });
    }
    else
    {
        master_table.modify(master_iter, _code, [&](auto &new_data) {
            new_data.state = _state;
        });
    }
}

void nftbay::checkpause(eosio::name _user)
{
    master_index master_table(_self, _self.value);
    auto master_iter = master_table.begin();
    eosio_assert(master_iter != master_table.end(), "Need Set Master");
    if(master_iter->state == system_state::pause)
    {
        white_index whitelist(_self, _self.value);
        auto white_iter = whitelist.find(_user.value);
        eosio_assert(white_iter != whitelist.end(), "Server pause.");
    }
}
...
extern "C"
{
    void apply(uint64_t receiver, uint64_t code, uint64_t action)
    {
        if (code == receiver)
        {
            eosio_assert(action != "bid"_n.value, "Impossible Action 1");
            eosio_assert(action != "transfer"_n.value, "Impossible Action 2");
            switch (action)
            {
                EOSIO_DISPATCH_HELPER(nftbay, (create)(transferid)(backtogame)(deletedata)(regauction)(cancelauc)(notify)(claim)(addwhitelist)(delwhitelist)(setpause))
            }
        }
        else if (code == "eosio.token"_n.value && action == "transfer"_n.value)
        {
            execute_action(name(receiver), name(code), &nftbay::bid);
        }
    }
};
//
// EOSIO_DISPATCH(nftbay, (create)(issue)(transferid)(backtogame)(clean)(regauction)(bid)(notify)(claim)(removetrade))
