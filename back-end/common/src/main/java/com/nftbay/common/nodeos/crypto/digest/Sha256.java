package com.nftbay.common.nodeos.crypto.digest;
...
public class Sha256 {
    ...
    public boolean equalsFromOffset(byte[] toCompareData, int offsetInCompareData, int len) {
        if ((null == toCompareData) || (offsetInCompareData < 0) || (len < 0) || (mHashBytes.length
                <= len) || (toCompareData.length <= offsetInCompareData)) {
            return false;
        }
